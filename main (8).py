...
def profile():
    ...
    # Получаем список пользователей в чёрном списке с деталями
    blacklist_users = rows_to_dicts(conn.execute('''
...
@app.route('/add_friend_to_blacklist/<int:friend_id>', methods=['GET', 'POST'])
def add_friend_to_blacklist(friend_id):
    ...
    try:
        ...
        # Удаляем из друзей (все записи между этими пользователями)
        conn.execute('''
        ...
        if existing:
            flash(f'{display_name} уже был в чёрном списке, удалён из друзей', 'info')
        else:
            flash(f'{display_name} добавлен в чёрный список и удалён из друзей', 'success')

    except Exception as e:
        conn.rollback()
        print(f"Ошибка add_friend_to_blacklist: {e}")
