...
def admin_add_news():
    ...
    u = conn_check.execute('SELECT role FROM users WHERE id = ?', (session['user_id'],)).fetchone()
    conn_check.close()
    actual_role = u['role'] if u else 'user'
    ...
    if not title or not body:
        ...
        return redirect('/home')
    conn = get_db_connection()
    conn.execute('INSERT INTO site_news (title, body) VALUES (?, ?)', (title, body))
    conn.commit()
    conn.close()
...
def home():
    ...
    if not _role:
        _u = conn3.execute('SELECT role FROM users WHERE id = ?', (user_id,)).fetchone()
        _role = _u['role'] if _u else 'user'
        session['role'] = _role
