...
def my_posts():
    ...
    # Загружаем медиафайлы для каждого поста
    for post in posts:
        ...
        for m in media_rows:
            md = dict(m)
            fp = os.path.join(app.config['POST_MEDIA_FOLDER'], md['filename'])
            md['exists'] = os.path.exists(fp)
            if not md.get('file_type'):
                ext = md['filename'].rsplit('.', 1)[-1].lower()
                if ext in ALLOWED_IMAGE_EXTENSIONS:
                    md['file_type'] = 'image'
                elif ext in ALLOWED_VIDEO_EXTENSIONS:
                    md['file_type'] = 'video'
                else:
                    md['file_type'] = 'document'
...
@app.route('/add_media_to_post/<int:post_id>', methods=['POST'])
def add_media_to_post(post_id):
    """Добавить медиафайл к существующему посту"""
    if 'user_id' not in session:
        return jsonify({'success': False}), 401
    user_id = session['user_id']
    conn = get_db_connection()
    try:
        ...
        unique_name = f"post_{post_id}_{int(time.time())}_{hashlib.md5(filename.encode()).hexdigest()[:8]}.{ext}"
        file.save(os.path.join(app.config['POST_MEDIA_FOLDER'], unique_name))
        conn.execute(
            'INSERT INTO post_media (post_id, filename, file_type, original_filename) VALUES (?,?,?,?)',
            (post_id, unique_name, ftype, file.filename)
        )
        conn.commit()
        return jsonify({'success': True})
    except Exception as e:
...
@app.route('/my_posts/delete_media/<int:media_id>', methods=['POST'])
def delete_post_media(media_id):
    """Удалить медиафайл из поста"""
    if 'user_id' not in session:
        return jsonify({'success': False}), 401
    user_id = session['user_id']
    conn = get_db_connection()
    try:
        media = conn.execute(
            'SELECT pm.*, p.user_id FROM post_media pm JOIN posts p ON p.id = pm.post_id WHERE pm.id = ?',
            (media_id,)
        ).fetchone()
        if not media:
...
def messenger_clear_history(partner_id):
    """Очистить историю переписки с пользователем"""
    if 'user_id' not in session:
        return jsonify({'success': False}), 401
    user_id = session['user_id']
    conn = get_db_connection()
...
def messenger_block(partner_id):
    """Добавить пользователя в чёрный список из мессенджера"""
    if 'user_id' not in session:
        return jsonify({'success': False}), 401
    user_id = session['user_id']
    if user_id == partner_id:
    ...
    try:
        ...
        if not existing:
            conn.execute(
                'INSERT INTO blacklist (blocker_id, blocked_id) VALUES (?, ?)',
                (user_id, partner_id)
            )
            conn.commit()
        return jsonify({'success': True})
